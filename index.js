...
<head>
  ...
  <style>
    body{font-family:Arial,sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}
    .card{background:#111827;border:1px solid #334155;border-radius:22px;padding:32px;max-width:520px;width:88%;text-align:center;box-shadow:0 20px 60px #0006}
    .spin{font-size:54px}.title{font-size:24px;font-weight:700;margin-top:12px}.sub{color:#cbd5e1;margin-top:12px;line-height:1.8}
  </style>
</head>
<body><div class="card"><div class="spin">⏳</div><div class="title">بانتظار QR من واتساب</div><div class="sub">الصفحة تتحدث تلقائياً كل 5 ثوانٍ.<br>الحالة: ${status}</div></div></body>
</html>`);
  }

  const qrTextJson = JSON.stringify(latestQr).replace(/</g, '\\u003c');
...
<head>
  ...
  <style>
    body{font-family:Arial,sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}
    .card{background:#111827;border:1px solid #334155;border-radius:22px;padding:28px;max-width:540px;width:90%;text-align:center;box-shadow:0 20px 60px #0006}
    .title{font-size:27px;font-weight:700;margin-bottom:8px}.sub{color:#cbd5e1;line-height:1.8;margin-bottom:18px}
    #qrWrap{display:inline-flex;background:#fff;padding:14px;border-radius:14px;min-width:300px;min-height:300px;align-items:center;justify-content:center}
    canvas{max-width:100%;height:auto}.note{font-size:14px;color:#94a3b8;margin-top:16px;line-height:1.7}.err{color:#fecaca;max-width:420px}
  </style>
</head>
<body>
  <div class="card">
    <div class="title">📱 ربط واتساب</div>
    <div class="sub">من الهاتف: واتساب ← الأجهزة المرتبطة ← ربط جهاز<br>وامسح هذا الرمز.</div>
    <div id="qrWrap"><canvas id="qr"></canvas></div>
    <div id="err" class="err"></div>
    <div class="note">الرمز يتجدد تلقائياً. إذا انتهت صلاحيته انتظر تحديث الصفحة.</div>
  </div>
  <script src="https://cdn.jsdelivr.net/npm/qrcode@1.5.4/build/qrcode.min.js"></script>
  <script>
    const qrText = ${qrTextJson};
    const canvas = document.getElementById('qr');
    const err = document.getElementById('err');

    if (window.QRCode && qrText) {
      QRCode.toCanvas(canvas, qrText, { width: 420, margin: 2 }, function (error) {
        if (error) {
          err.textContent = 'تعذر رسم QR. حدّث الصفحة بعد ثوانٍ.';
          console.error(error);
        }
      });
    } else {
      err.textContent = 'تعذر تحميل أداة QR. حدّث الصفحة أو افتحها بمتصفح Chrome.';
    }
  </script>
</body>
</html>`);
