...
function normalizeArabic(value) {
  return String(value || '')
    ...
    .trim()
    .toLowerCase()
    .replace(/[\u200B-\u200F\u202A-\u202E\u2066-\u2069\uFEFF]/g, '') // محارف مخفية واتجاه النص
    .replace(/[\u064B-\u065F\u0670\u06D6-\u06ED]/g, '') // الحركات
    .replace(/\u0640/g, '') // التطويل
    .replace(/[أإآٱ]/g, 'ا')
    .replace(/ى/g, 'ي')
    .replace(/ة/g, 'ه')
    .replace(/[\"'`´“”‘’]/g, '') // علامات اقتباس قد تُنسخ داخل Environment
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function isTargetGroup(message) {
  ...
  const actual = normalizeArabic(groupName);
  const configured = normalizeArabic(TARGET_GROUP_NAME);
  // اسم الكروب المؤكد من المستخدم. نخليه fallback حتى لو قيمة Render بيها اقتباس/محرف مخفي/تهجئة قديمة.
  const confirmed = normalizeArabic('الموافقات الامنيه');

  const ok = actual === configured || actual === confirmed;

  if (!ok) {
    console.log('🔎 اسم الكروب الفعلي:', JSON.stringify(groupName));
    console.log('🔎 TARGET_GROUP_NAME:', JSON.stringify(TARGET_GROUP_NAME));
    console.log('🔎 normalized actual:', JSON.stringify(actual));
    console.log('🔎 normalized target:', JSON.stringify(configured));
  }
...
async function connectToWhatsApp() {
  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      // بعض الرسائل المرسلة من الهاتف الأساسي للحساب المرتبط تصل كـ append،
      // لذلك نقبل notify و append، لكن نمنع سجل المحادثات القديم بالوقت.
      if (type !== 'notify' && type !== 'append') return;

      const list = messages || [];
      console.log(`📨 messages.upsert: ${type} — count: ${list.length}`);
      ...
      for (const message of list) {
        if (type === 'append') {
          ...
          const seconds = typeof raw === 'number'
            ? raw
            : Number(raw?.low ?? raw ?? 0);
          const messageMs = Number.isFinite(seconds) && seconds > 0
            ? seconds * 1000
            : 0;

          // نتجاهل فقط append القديم أكثر من دقيقتين حتى لا نعيد صور history.
          if (!messageMs || messageMs < Date.now() - (2 * 60 * 1000)) {
            continue;
          }
        }
